 ...
 */
goog.provide('bzl.ui.Timeline');
goog.provide('bzl.ui.TimelineEventSource');
 ...
 * @param {number=} width
 */
bzl.ui.Timeline = function(width) {
...
goog.inherits(bzl.ui.Timeline, goog.ui.Component);


/**
 * @param {number} width
 */
bzl.ui.Timeline.prototype.setWidth = function(width) {
  this.width = width;
  ...
  if (this.source && this.sourceData) {
    // this.debug('set width to ' + width + ' and reupdating');
    this.update(this.source, this.sourceData);
  } else {
    // this.debug('set width to ' + width + ' and NOT reupdating');
  }
};
 ...
 * @override
 */
bzl.ui.Timeline.prototype.enterDocument = function() {
  bzl.ui.Timeline.superClass_.enterDocument.call(this);
  ...
  var dt_get = function(event) {
    var date = self.source.getEventDate(event);
    // console.log('event: ' + event + ', date = ' + date);
    return date;
  };

  var yr_get = function(event) {
    return self.source.getEventYear(event);
  };
  var color = function(event) {
    return self.source.getEventColor(event) || 'grey';
  };
  ...
  this.update = function(source, data) {
